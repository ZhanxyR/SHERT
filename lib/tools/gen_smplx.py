...
def init_smplx_model(source, gender='neutral'):
...
def gen_star_smplx(smplx_param_path, save_root, cfg_resources):
    ...
    if 'translation' in smplx_param.keys():
        ...
        source = 'econ'
        for key in smplx_param.keys():
            smplx_param[key] = smplx_param[key].reshape(1, -1)
        translation = smplx_param['transl']

    smplx_model = init_smplx_model(source)
    ...
    smplx_verts = (smplx_out.vertices[0] * smplx_param['scale'].detach() + translation).detach().numpy()
